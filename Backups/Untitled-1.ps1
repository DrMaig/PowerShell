...
<#
.SYNOPSIS
    Platform detection and global environment flags
.DESCRIPTION
    Detects and classifies the runtime environment including OS platform,
    user context, and execution environment for adaptive behavior.
#>

# Platform detection with fallback
try {
    Set-Variable -Name IsWindows -Value ([System.Runtime.InteropServices.RuntimeInformation]::IsOSPlatform([System.Runtime.InteropServices.OSPlatform]::Windows)) -Scope Global -Force
    Set-Variable -Name IsLinux -Value ([System.Runtime.InteropServices.RuntimeInformation]::IsOSPlatform([System.Runtime.InteropServices.OSPlatform]::Linux)) -Scope Global -Force
    Set-Variable -Name IsMacOS -Value ([System.Runtime.InteropServices.RuntimeInformation]::IsOSPlatform([System.Runtime.InteropServices.OSPlatform]::OSX)) -Scope Global -Force
} catch {
    # Fallback for older hosts
    Set-Variable -Name IsWindows -Value ($env:OS -like '*Windows*') -Scope Global -Force
...
if (-not $Global:ProfileConfig) {
    $Global:ProfileConfig = [ordered]@{
        ...
        DeferredLoader   = @{
            ...
            Modules           = @('Terminal-Icons', 'posh-git', 'PSFzf', 'z')
            UseJobs           = $true
            MaxParallelJobs   = 3
            TimeoutSeconds    = 30
        }
        ...
        ToolchainCatalog = @{
            CatalogPath      = $null
            CacheFile        = 'toolchain_catalog.json'
            CacheMaxDays     = 30
            CatalogUpdateUrl = $null
            TrendingRatio    = 0.4
            PopularRatio     = 0.6
        }
        ...
        UtilityDefaults  = @{
            ...
            ServiceHealthFilter       = 'Running'
            FindTopDiskConsumersTop   = 10
            TestTcpPortTimeoutMs      = 1200
            NetworkTestTimeoutSec     = 3
        }
        ...
        PSReadLine       = @{
            EditMode                  = 'Windows'
            HistorySize               = 10000
            HistorySavePath           = $null  # Set later
            ...
            KeyBindings               = @{
                ...
                'Tab'              = 'MenuComplete'
                'Ctrl+Tab'         = 'AcceptNextSuggestionWord'
...
# Profile Statistics
$Global:ProfileStats = [PSCustomObject]@{
    CommandsExecuted = 0
    ErrorsEncountered = 0
    WarningsIssued = 0
    ModulesLoaded = 0
    SessionUptime = [TimeSpan]::Zero
}

# Profile Version
$script:ProfileVersion = '2026.02.09-ps7.5'
$script:ProfileChangelog = @(
    'Comprehensive PowerShell 7.5+ profile with enterprise features',
    'Runtime guards and environment validation',
    'Advanced logging with structured output',
    'Performance monitoring and benchmarking',
    'System administration and optimization functions',
    'Network management with DNS profiles',
    'Self-diagnostics and troubleshooting',
    'PSReadLine with predictive IntelliSense',
    'Deferred module loader for performance',
    'Third-party CLI tool integrations'
)
...
#region 18 - ALIASES AND SHORTCUTS
#==============================================================================
<#
.SYNOPSIS
...
# Navigation aliases
Set-Alias -Name .. -Value 'cd ..' -Option AllScope -ErrorAction SilentlyContinue
Set-Alias -Name ... -Value 'cd ../..' -Option AllScope -ErrorAction SilentlyContinue
Set-Alias -Name ~ -Value 'cd ~' -Option AllScope -ErrorAction SilentlyContinue
...
#region 19 - THIRD-PARTY CLI TOOL INTEGRATIONS
#==============================================================================
<#
.SYNOPSIS
    Third-party CLI tool integrations
...
function Initialize-ThirdPartyTools {
    <#
    .SYNOPSIS
        Initializes third-party tool integrations.
    .DESCRIPTION
        Sets up completions and integrations for installed CLI tools.
    #>
    [CmdletBinding()]
    param()
    ...
    if (Get-Command winget -ErrorAction SilentlyContinue) {
        try {
            ...
            Write-ProfileLog "winget completions registered" -Level DEBUG -Component "Tools"
        } catch {}
    }
    ...
    if (Get-Command gh -ErrorAction SilentlyContinue) {
        try {
            Invoke-Expression -Command $(gh completion -s powershell | Out-String) -ErrorAction SilentlyContinue
            Write-ProfileLog "gh completions registered" -Level DEBUG -Component "Tools"
        } catch {}
    }
    ...
    if (Get-Command kubectl -ErrorAction SilentlyContinue) {
        try {
            ...
            Write-ProfileLog "kubectl completions registered" -Level DEBUG -Component "Tools"
        } catch {}
    }
    ...
    if (Get-Command helm -ErrorAction SilentlyContinue) {
        try {
            helm completion powershell | Out-String | Invoke-Expression -ErrorAction SilentlyContinue
            Write-ProfileLog "helm completions registered" -Level DEBUG -Component "Tools"
        } catch {}
    }

    # dotnet completion
    if (Get-Command dotnet -ErrorAction SilentlyComplete) {
        try {
            ...
            Write-ProfileLog "dotnet completions registered" -Level DEBUG -Component "Tools"
        } catch {}
    }
}

# Initialize third-party tools for interactive sessions
if (Test-ProfileInteractive) {
    Initialize-ThirdPartyTools | Out-Null
}

#endregion THIRD-PARTY CLI TOOL INTEGRATIONS


#region 20 - WELCOME SCREEN AND FINALIZATION
#==============================================================================
<#
.SYNOPSIS
...
#region 21 - EXIT HANDLERS AND CLEANUP
#==============================================================================
<#
.SYNOPSIS
